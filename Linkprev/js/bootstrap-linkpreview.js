...
(function($) {
    ...
    LinkPreview.prototype = {
        ...
        findDescriptionInDom: function($dom) {
            return $dom.find("meta[name=description]").attr("content") || $dom.find("meta[property='og:description']").attr("content");
        },

        findImageInDom: function($dom) {
            if($dom.find("link[rel=icon]")[0] != undefined){
             var hrefIco = $dom.find("link[rel=icon]").sort(x=>x.sizes)[$dom.find("link[rel=icon]").length-1].href.split("/");
             var iconScrap =  "/"+hrefIco.slice(3, hrefIco.length).join("/")
            }
